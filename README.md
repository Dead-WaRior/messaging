...
## Full QA flow (Task 19)

Signup → login → edit profile → create post (text + image) → view feed → open a post → comment → like → open another user's profile → follow → switch to Following tab → logout → log back in → all state persists → try to delete someone else's post/comment (should fail 403) → tamper with token in localStorage (should redirect to login on next auth call).
